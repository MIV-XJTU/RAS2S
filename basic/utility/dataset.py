...
class _AddNoisePoisson(object):
    def __init__(self, lambdas):
        self.lambdas = np.array(lambdas)

    def __call__(self, img, bands):
        bw_lambdas = self.lambdas[np.random.randint(0, len(self.lambdas), len(bands))]
        for i, lam in zip(bands, bw_lambdas):
            self.add_noise(img[i, ...], lam=lam)
        return img

    def add_noise(self, image, lam):
        noisy_image = np.random.poisson(lam=lam, size=image.shape)
        return image + noisy_image

class _AddNoiseSpeckle(object):
    def __init__(self, variances):
        self.variances = np.array(variances)

    def __call__(self, img, bands):
        bw_variances = self.variances[np.random.randint(0, len(self.variances), len(bands))]
        for i, var in zip(bands, bw_variances):
            self.add_noise(img[i, ...], var=var)
        return img

    def add_noise(self, image, var):
        noisy_image = np.random.normal(scale=np.sqrt(var), size=image.shape)
        return image + noisy_image
...
class AddNoisePoisson(AddNoiseMixed):
    def __init__(self):
        self.noise_bank = [_AddNoisePoisson([0.1, 0.3, 0.5, 0.7])]
        self.num_bands = [1/3]

class AddNoiseSpeckle(AddNoiseMixed):
    def __init__(self):
        self.noise_bank = [_AddNoiseSpeckle([0.01, 0.05, 0.1, 0.2])]
        self.num_bands = [1/3]
...
class AddNoiseComplex2(AddNoiseMixed):
    def __init__(self):
        self.noise_bank = [
            _AddNoiseStripe(0.05, 0.15), 
            _AddNoiseDeadline(0.05, 0.15),
            _AddNoiseImpulse([0.1, 0.3, 0.5, 0.7]),
            _AddNoiseSpeckle([0.01, 0.05, 0.1, 0.2]),
            _AddNoisePoisson([0.1, 0.3, 0.5, 0.7])
        ]
        self.num_bands = [1/5, 1/5, 1/5]
...
def addNoiseComplex2(srcdir,dstdir):
    sigmas = [10,30,50,70]
    noise_models = []
    names = ['blind_gaussian','poisson','add_Speckle_noise','mixture']
    noise_models.append(AddNoiseBlind(sigmas))
    add_Poisson_noise = Compose([
        AddNoiseNoniid(sigmas),
        AddNoisePoisson(),
    ])
    noise_models.append(add_Poisson_noise)
    add_Speckle_noise = Compose([
        AddNoiseNoniid(sigmas),
        AddNoiseSpeckle(),
    ])
    noise_models.append(add_Speckle_noise)
...
if __name__ == '__main__':
    srcdir = '/data1/jiahua/ly/test_data/wdc/'   #file path of original file
    dstdir = '/data1/jiahua/ly/test_data/wdc_added_noise/' #file path to put the testing file+
    # addNoiseComplex(srcdir,dstdir)
    addNoiseComplex2(srcdir,dstdir)
